...
export class CustomLinkComponent extends LinkModel implements AfterViewInit {
  ...
  calcCenterOfPath(): Coords {
    ...
    return {
      x: (firstPointCoords.x + lastPointCoords.x) / 2,
      y: (firstPointCoords.y + lastPointCoords.y) / 2,
    };
  }

  selectPath(): Observable<string | null> {
